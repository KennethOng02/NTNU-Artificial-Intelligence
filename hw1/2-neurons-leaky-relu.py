...
class NeuralNetwork():
    ...
    def forward_propagation(self, Z):
        layer1_out = self.leaky_relu(np.dot(Z, self.layer1.weights) + self.layer1.bias)
        layer2_out = self.leaky_relu(np.dot(layer1_out, self.layer2.weights) + self.layer2.bias)
    ...
    def train(self, train_in, train_sol):
        ...
        for i in range(100000):
            ...
            layer2_delta = layer2_error * self.leaky_relu_derivative(layer2_out)

            layer1_error = layer2_delta.dot(self.layer2.weights.T)
            layer1_delta = layer1_error * self.leaky_relu_derivative(layer1_out)
